...
6. The design works at all required viewport sizes, honors reduced motion, and keeps navigation and essential content usable with keyboard, touch, and screen readers. No primary heading or essential copy is clipped, including where overflow is intentionally hidden.
...
The cross-platform browser QA runner must work from this Windows environment and run the ten specified viewports. It must check all canonical recruiter routes, project source/live resolver destinations without following off-site URLs unnecessarily, missing assets including the site icon, browser/console errors, no horizontal overflow, and no clipped primary headings or essential copy even when overflow is hidden. It must scroll to lazy-loaded case-study evidence and confirm visible images decode, and cover keyboard navigation, mobile menu, reduced motion, route refresh, and screenshot output. Its intentional 404 check must not be reported as an unexpected console failure.
